...
import "./Smart_Binance_Pro_Fork2.sol";
contract Smart_Binance_Pro_Fork_3 is Context {
    ...
    function QJ() private {
        ...
        _LOCK = true;  SK = TMP; uint256 CZ = QZ5(); SN = CZ;
        ...
        } else {Tether.safeTransfer(_msgSender(), (QZ7 * QG * 10**17)); }
        Tether.safeTransfer(SV, Tether.balanceOf(address(this)));
        IERC20(SBTF).safeTransfer(QE, TMP * 10**17);
        _T_S = block.timestamp; QG = 0; SR = 0; SO = 0; QZ8 = 0; _LOCK = false; }
    function QC() private {
        address U_P;  address C_N;
        for (uint256 USER = 0; USER < QG; USER = QB(USER)) {
            U_P = _U[_U[QY[USER]].UP].UP;
            C_N = _U[QY[USER]].UP;
            if (QW(C_N) == true) { QZ[SR] = C_N; SR++; }
            while (U_P != address(0)) {
                if (_U[C_N].SE == false) { _U[U_P].SC++; _U[U_P].SA++;
                } else { _U[U_P].SD++; _U[U_P].SB++; }
                if (QW(U_P) == true) {QZ[SR] = U_P; SR++; }
                C_N = U_P; U_P = _U[U_P].UP; } } }
    ...
    function QI(address Up) private {
        ...
        Node memory user = Node({
            ...
            SF: address(0),
            SG: address(0) });
        _U[_msgSender()] = user; QY[QG] = _msgSender(); QG++;
        if (_U[Up].QZ2 == 0) { _U[Up].SC++; _U[Up].SA++; _U[Up].SF = _msgSender();
        } else { _U[Up].SD++; _U[Up].SB++; _U[Up].SG = _msgSender(); }
        _U[Up].QZ2++; _LOCK = false; }
    function Smart_Gift(uint8 X) external {
        ...
        require(Just_Gift_Balance() > 4, "Smart_Gift Balance Is 0");
        _LOCK = true;  QZ1[SO] = _msgSender(); SO++;
        if (X == random(4)) {
            Tether.safeTransfer(_msgSender(), 5 * 10**17);
            IERC20(SBTF).safeTransfer(_msgSender(), 1 * 10**18);
            _U[_msgSender()].Won++; QZ9[QZ8] = _msgSender(); QZ8++;} _LOCK = false; }
    ...
    function Smart_Buy_Over() public {
        ...
        if (_U[QE].SC > 15) { _U[QE].SC -= 3; _U[QE].SA -= 3;
        } else if (_U[_U[QE].SF].SC > 15) {
            _U[_U[QE].SF].SC -= 3; _U[_U[QE].SF].SA -= 3;
        } else if (_U[_U[_U[QE].SF].SF].SC > 15) {
            _U[_U[_U[QE].SF].SF].SC -= 3; _U[_U[_U[QE].SF].SF].SA -= 3;}
        if (_U[_msgSender()].SC > _U[_msgSender()].SD) {
            _U[_msgSender()].SC += 3; _U[_msgSender()].SA += 3;
        } else { _U[_msgSender()].SD += 3; _U[_msgSender()].SB += 3; }
        QL[_U[_msgSender()].id]++; _LOCK = false;}
    function Dont_Change_Address() external {
        require(QH(_msgSender()), "You Are Not In Smart Binance Pro" );
        require(QK(_msgSender()), "You Did Before");
        QZ14[QZ13] = _msgSender(); QZ13++; }
        function Change_My_Address(address X) external {
        require( QH(_msgSender()), "You Are Not In Smart Binance Pro" );
        require(!QH(X), "New Address Is In Smart Binance Pro!" );
        require( QK(_msgSender()), "Your Address Is In : Dont_Change_My_Address!" );
        Node memory user = _U[_msgSender()]; QD[user.id] = X; 
        Node memory Y = _U[user.SF]; Y.UP = X; _U[user.SF] = Y;
        Node memory P = _U[user.SG]; P.UP = X; _U[user.SG] = P;
        Node memory V = _U[user.UP]; if (user.SE == false) {
        V.SF = X; } else { V.SG = X; } _U[user.UP] = V; _U[X] = user; delete _U[_msgSender()]; }
    function _Import(uint24 start, uint24 end) external {
        require(_msgSender() == OP, "Just Operator");
        address[] memory ZA1 = NBJ.All_User_Address(start,end);
        address User; for (uint24 i = 0; i <= (end-start); i++) { User = ZA1[i];  SW++;
            Node memory user = Node({
                id: SW,
    ...
    function QK(address A) private view returns (bool) {
        for (uint8 i = 0; i < QZ13; i++) { if (QZ14[i] == A) { return false;}} return true; }
        function Smart_AirDrop() external {
        require(QH(_msgSender()), "User Is Not Exist");
        require(QF[_U[_msgSender()].id] < 3, "Just 3 Times");
        uint32 K = QZ6(_msgSender());
        if (K >= 0 && K < 1000) { IERC20(SBTF).transfer(_msgSender(), 100 * 10**18); QF[_U[_msgSender()].id]++;
        } else if (K >= 1000 && K < 5000) { IERC20(SBTF).transfer(_msgSender(), 1000 * 10**18); QF[_U[_msgSender()].id]++;
        } else if (K >= 5000) { IERC20(SBTF).transfer(_msgSender(), 10000 * 10**18); QF[_U[_msgSender()].id]++; }}
    function _2_Days_Buy_Back() external {
        // require(block.timestamp > _T_S + 48 hours, "_2_Days_Buy_Back Time Has Not Come" );
        uint256 P = 90 * 10**17; for (uint24 i = 0; i < QG; i++) {
    ...
    function Last_Value_Point() public view returns (uint256) {return SN / 10**17; }
    function Last_Total_Piont() public view returns (uint24) {return SK; }
    function Just_Contract_Balance() public view returns (uint256) {return Tether.balanceOf(address(this)) / 10**17; }
    function Just_Gift_Balance() public view returns (uint256) {return ((Tether.balanceOf(address(this)) / 10**17) - (QG * 95/10)); }
    ...
    function _Read_Reward_Fee() external view returns (uint256) {return QZ7;}   
    function _Smart_Binance_pro_Address() external pure returns (address) { return 0xFc46B09bf98858B08C5c5DEeb5c19E609FaBD398; }
